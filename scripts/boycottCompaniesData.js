 ...
 * 
 * This script contains data for the boycottCompanies collection and provides
 * functions to add this data to Firebase Firestore.
 */

// Import Firebase Admin SDK for direct upload to Firestore
const admin = require('firebase-admin');
const serviceAccount = require('../firebase-service-account.json');

// Data for boycottCompanies collection
const boycottCompaniesData = [
  {
    "id": "1",
    "name": "Espressolab",
...
/**
 * Method 1: Using Firebase Admin SDK (for server-side or scripts)
 * Use this approach for initialization scripts or backend operations
 */
async function uploadToFirestoreWithAdminSDK() {
  try {
    ...
    for (const company of boycottCompaniesData) {
      ...
      if (company.startDate) {
        company.startDate = admin.firestore.Timestamp.fromDate(new Date(company.startDate));
      }
      ...
      // Add to batch
      const docRef = db.collection('boycottCompanies').doc(company.id);
      batch.set(docRef, docWithTimestamps);
    }
...
/**
 * Method 2: Using Firebase Client SDK (for client-side applications)
 * Import this in your application for client-side usage
 */
// Import these in your client app
// import { collection, addDoc, setDoc, doc, serverTimestamp } from 'firebase/firestore';
// import { firestore } from '../services/firebase';

async function uploadBoycottCompaniesToFirestore(firestore) {
  try {
    // Process each company
    for (const company of boycottCompaniesData) {
      // Convert date string to JavaScript Date (Firestore will handle it)
      const processedCompany = { ...company };
      if (processedCompany.startDate) {
...
// Uncomment to execute direct upload with Admin SDK
uploadToFirestoreWithAdminSDK().then(() => process.exit(0));

// Export the data for use in other files
module.exports = {
  boycottCompaniesData,
  uploadBoycottCompaniesToFirestore
}; 